...
<?php
session_start();
    $amount = intval(substr($_GET['total'],3))*100; // convert the amount to paisa as the amount should be in paisa for khalti
    $purchase_order_id = 5;
   ...
   if(empty($amount) || empty($name) || empty($email) || empty($phone)){
    ...
    $_SESSION['orderPhone']=$phone;
    $_SESSION['orderAddress']=$_GET['address'];
...
$postFields = array(
    "return_url" => "http://localhost/NepaliFootprints/Khalti/payment-response.php",
    "website_url" => "http://localhost/NepaliFootprints/",
    "amount" => $amount,
